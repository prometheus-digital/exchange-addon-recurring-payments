jQuery( document ).ready( function ( $ ) {
	...
	$( "#recurring-payments-save" ).click( function ( e ) {
		...
		$( '.recurring-options' ).each( function () {
			...
			$.ajax( {
				...
				error  : function ( xhr ) {
					alert( itExchange.common.getErrorFromXhr( xhr ) );
				},
			} );
		} );
	...
	$( "#comp-subscription" ).click( function ( e ) {
		e.preventDefault();

		$( ".transaction-actions" ).slideUp();
		$( '#subscription-comp-manager' ).slideDown();
	} );

	$( "#cancel-comp-subscription" ).click( function ( e ) {

		e.preventDefault();

		$( ".transaction-actions" ).slideDown();
		$( '#subscription-comp-manager' ).slideUp();
	} );

	$( '#confirm-comp-subscription' ).click( function ( e ) {
		e.preventDefault();
		...
		$.ajax( {
			type   : 'POST',
			url    : $( "#comp-subscription" ).data( 'route' ),
			data   : {
				comped: EXCHANGE_RP.user,
				reason: $( "#comp-subscription-reason" ).val(),
			},
			success: function ( data ) {
				wp.heartbeat.interval( 'fast', 6 );
				$( '#rp-status-' + data.product ).val( data.status.slug );
