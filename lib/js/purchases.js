(function ( $, api, common, config ) {
	...
	for ( var i = 0; i < config.subscriptions.length; i++ ) {
		subscription = new api.Models.Subscription( config.subscriptions[i] );
		subscription.upgrades().set( config.upgrades[subscription.id] );
		subscription.downgrades().set( config.downgrades[subscription.id] );
	...
	$( document ).ready( function () {
		...
		changeSubscription.start( { subscriptions: subscriptions } );
	...
	var changeSubscription = window.ITExchangeChangeMySubscription = {
		...
		start: function ( options ) {
			for ( var subID in options.subscriptions ) {
				...
				if ( !subscription.upgrades().length && !subscription.downgrades().length ) {
					continue;
				}

				var view = new changeSubscription.Views.ChangeMySubscription( {
					model: subscription
				} );
				view.inject(
					'#it-exchange-change-subscription-' + subscription.get( 'transaction' ) + '-' + subscription.get( 'product' )
				);
			}
		}
	};

	changeSubscription.Views.ChangeMySubscription = api.View.extend( {
		template: wp.template( 'it-exchange-change-my-subscription' ),

		events: {
			'click .it-exchange-change-my-subscription--trigger': 'begin'
		},

		initialize: function ( options ) {

			_.each( this.model.upgrades().models, (function ( model ) {
				this.addOfferView( model, 'upgrade' );
			}).bind( this ) );

			_.each( this.model.downgrades().models, (function ( model ) {
		...
		render: function () {
			...
			this.$( '.it-exchange-subscription-updowngrade-options' ).hide();
			this.views.render();
		},

		begin: function () {
			this.$( '.it-exchange-change-my-subscription--trigger' ).hide();
			this.$( '.it-exchange-subscription-updowngrade-options' ).show();

			if ( renewSubscription.cartLoaded ) {
				this.$( '.it-exchange-change-my-subscription-offer--trigger' ).removeAttr( 'disabled' );
			} else {
		...
		addOfferView: function ( model, type ) {
			this.views.add( '.it-exchange-subscription-updowngrade-options', new changeSubscription.Views.Offer( {
				model: model,
				type : type,
			} ) )
		},
	...
	changeSubscription.Views.Offer = api.View.extend( {
		template : wp.template( 'it-exchange-change-my-subscription-offer' ),
		tagName  : 'li',
		className: 'it-exchange-subscription-updowngrade-item',
		events   : {
			'click .it-exchange-change-my-subscription-offer--trigger': 'continueToPayment',
			'click. .it-exchange-change-my-subscription-offer--cancel': 'cancelPayment'
		},
		...
		render: function () {
			...
			this.$( '.it-exchange-change-my-subscription-offer--cancel' ).hide();
		...
		continueToPayment: function ( e ) {
			...
			this.$( '.it-exchange-change-my-subscription-offer--cancel' ).show();

			this.model.accept( null, { embed: true } ).done( (function ( cart ) {

				this.$( '.it-exchange-change-my-subscription-offer--trigger' ).hide();

				this.cart = cart;

				var attr = { model: this.cart };
				attr.defaultPurchaseMethod = this.model.parentModel.get( 'payment_method.method.slug' );

				var checkout = new api.Views.Checkout( attr );
				checkout.inject( this.$( '.it-exchange-change-my-subscription-offer--checkout-container' ) );
			}).bind( this ) );
		},

		cancelPayment: function () {

			this.$( '.it-exchange-change-my-subscription-offer--trigger' ).removeAttr( 'disabled' ).show();
			this.$( '.it-exchange-change-my-subscription-offer--cancel' ).hide();
			this.$( '.it-exchange-change-my-subscription-offer--checkout-container' ).remove();
	...
	renewSubscription.Views.Renew = api.View.extend( {
		...
		renew: function () {
			...
			if ( this.checkout ) {
				...
				this.$( '.it-exchange-renew-subscription' ).hide().prop( 'disabled', false );
				this.$( '.it-exchange-renew-subscription--cancel' ).show();

				return;
			}

			this.model.customer().tokens().doFilter( { gateway: this.model.get( 'payment_method.method.slug' ) }, {
				local : true,
				greedy: true
			} );
			this.model.renew( null, { embed: true } ).done( (function ( cart ) {
				this.$( '.it-exchange-renew-subscription' ).hide().prop( 'disabled', false );
				this.$( '.it-exchange-renew-subscription--cancel' ).show();
				...
				var attr = {
					...
					defaultPurchaseMethod: this.model.get( 'payment_method.method.slug' ),
				};

				// For non-auto-renewing subscriptions, preselect the payment token last used. We don't do this for
				// auto-renewing subscriptions, because if the subscription is no longer active it is most likely
				// because that card is no longer working.
				if ( !this.model.get( 'auto_renewing' ) && this.model.has( 'payment_method.token' ) ) {
					attr.defaultPurchaseToken = this.model.get( 'payment_method.token' );
				}

				this.checkout = new api.Views.Checkout( attr );
	...
	var updatePaymentMethod = window.ITExchangeUpdateSubscriptionPaymentMethod = {
		...
		start: function ( options ) {
			for ( var subID in options.subscriptions ) {
				...
				if ( ! subscription.has( 'payment_method.token') && ! subscription.has( 'payment_method.card' ) ) {
					continue;
				}
	...
	updatePaymentMethod.Views.PaymentMethodSource = api.View.extend( {
		...
		onSave: function ( e ) {
			...
			if ( this.visualCC ) {
				...
				var gateway = this.model.get( 'payment_method.method.slug' );
		...
		createToken: function ( tokenize ) {
			this.model.customer().tokens().create( {
				gateway: this.model.get( 'payment_method.method.slug' ),
				source : tokenize,
			}, {
				wait   : true,
...
})( jQuery, window.itExchange.api, window.itExchange.common, window.ITExchangeRecurringPayments );