jQuery( document ).ready( function ( $ ) {
	...
	$( '#it-exchange-recurring-payment-settings' ).on( 'change', '#it-exchange-recurring-payments-auto-renew', function () {
		...
		$( "#sign-up-fee-settings" ).toggleClass( 'hidden' );
	...
	$( '#it-exchange-recurring-payments-sign-up-fee' ).on( 'focusout', function () {
		var fee = Number.parseFloat( itExchange.common.getFloatFromString( $( this ).val() ) ), min;

		if ( $( '#it-exchange-recurring-payments-trial-enabled' ).is( ':checked' ) ) {
			min = 0;
		} else {
			min = itExchange.common.getFloatFromString( $( '#base-price' ).val() ) * -1;
		}
		...
		var formatted = fee === 0 ? '' : itExchange.common.formatPrice( fee );
		$( this ).val( formatted );
	} );
