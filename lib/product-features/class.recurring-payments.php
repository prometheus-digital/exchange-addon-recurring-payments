	...
	function print_metabox( $post ) {
        ...
        <div id="it-exchange-recurring-payment-settings">
	        ...
	        <div id="recurring-payment-options" class="<?php echo $hidden; ?>">
				...
				?>
		        <div id="trial-period-options" class="<?php echo $trial_hidden; ?>">
		        ...
		        <select id="it-exchange-recurring-payments-trial-interval" name="it_exchange_recurring_payments_trial_interval">
				    ...
				    foreach( $interval_types as $name => $label ) {
					    echo '<option value="' . $name . '" ' . checked( $trial_interval, $name, false ) . '>' . $label . '</option>';
				    }  
					?>
		        </select>
		        </div>
		        ...
		        <label for="it-exchange-recurring-payments-trial-interval">
			        <?php _e( 'Recurs every...', 'LION' ); ?>
		        </label>
		        &nbsp;
		        <input id="it-exchange-recurring-payments-trial-interval" type="number" class="small-input" name="it_exchange_recurring_payments_trial_interval" value="<?php echo $interval_count; ?>" placeholder="#" />
		...
		<?php
		/* CHANGEME delete this before tagging
        <div id="it-exchange-recurring-payment-settings">
	        <label for="it-exchange-recurring-payments-enabled"><?php _e( "Enable Recurring Payments?", 'LION' ); ?>
	        	<input id="it-exchange-recurring-payments-enabled" type="checkbox" name="it_exchange_recurring_payments_enabled" <?php checked( $enabled ); ?> />
	        </label>
	        ...
	        <?php
		        ...
		        $trial_input_interval_count = '<input id="it-exchange-recurring-payments-trial-interval-count" type="number" class="small-input" name="it_exchange_recurring_payments_trial_interval_count" value="' . $trial_interval_count . '" placeholder="#" />';
		        $trial_input_interval = '<select id="it-exchange-recurring-payments-trial-interval" name="it_exchange_recurring_payments_trial_interval">';
			    foreach( $interval_types as $name => $label ) {
				    $trial_input_interval .= '<option value="' . $name . '" ' . selected( $trial_interval, $name, false ) . '>' . $label . '</option>';
			    }  
			    $trial_input_interval .= '</select>';

		        $input_interval_count = '<input id="it-exchange-recurring-payments-trial-interval-count" type="number" class="small-input" name="it_exchange_recurring_payments_interval_count" value="' . $interval_count . '" placeholder="#" />';
		        $input_interval = '<select id="it-exchange-recurring-payments-interval" name="it_exchange_recurring_payments_interval">';
			    foreach( $interval_types as $name => $label ) {
				    $input_interval .= '<option value="' . $name . '" ' . selected( $interval, $name, false ) . '>' . $label . '</option>';
			    }  
			    $input_interval .= '</select>';
			    
		        $auto_renew_select  = '<select id="it-exchange-recurring-payments-auto-renew" name="it_exchange_recurring_payments_auto_renew">';
		        $auto_renew_select .= '<option value="on" ' . selected( 'on', $auto_renew, false ) . '>' . __( 'is', 'LION' ) . '</option>';
		        $auto_renew_select .= '<option value="off" ' . selected( 'off', $auto_renew, false ) . '>' . __( 'is not', 'LION' ) . '</option>';
		        $auto_renew_select .= '</select>';

		        printf( __( 'This product is free for %s %s then recurs every %s %s and %s auto-renewing', 'LION' ), $trial_input_interval_count, $trial_input_interval, $input_interval_count, $input_interval, $auto_renew_select );
		    ?>
		        </select>
	        </div>
        </div>
